  ...
  return (
    <div ref={ref} className={`relative ${className}`}>
      ...
      <button
        ...
        className="w-full flex items-center justify-between gap-2 px-3.5 py-2.5 rounded-xl bg-surface-2 hover:bg-surface-3 border border-white/10 hover:border-white/20 text-xs font-medium text-white transition-all text-left outline-none focus-visible:ring-2 focus-visible:ring-coral-500/50"
      >
        <div className="flex items-center gap-2 truncate">
          {selectedCategory && selectedCategory.slug !== 'all' ? (
            <>
              <CategoryIcon name={selectedCategory.slug} className="w-4 h-4 text-coral-400 flex-shrink-0" />
      ...
      {open && (
        <div className="absolute left-0 right-0 top-full mt-1.5 z-50 max-h-64 overflow-y-auto rounded-xl bg-surface-2 border border-white/10 shadow-2xl p-1.5 scrollbar-none no-scrollbar backdrop-blur-xl animate-fade-in">
          {categories.map((cat) => {
