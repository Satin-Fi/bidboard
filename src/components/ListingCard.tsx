...
import { toggleWatch } from '../store/marketActions'
...
export default function ListingCard({ listing }: { listing: Listing }) {
  ...
  return (
    <Link to={`/listing/${listing.id}`} className="card overflow-hidden group hover:border-accent/40 transition relative">
      {Date.now() - listing.createdAt < 24 * 60 * 60 * 1000 && (
        <span className="absolute top-3 left-3 z-10 chip bg-accent-2/20 text-accent-2 border border-accent-2/30">New</span>
      )}
      ...
      <button
        aria-label="Watch"
        onClick={(e) => { e.preventDefault(); e.stopPropagation(); toggleWatch(listing.id, !watched) }}
      ...
      <div className={`relative h-40 bg-gradient-to-br ${listing.gradient}`}>
        ...
        <div className="absolute bottom-3 left-3 chip bg-black/40 text-white">{listing.category}</div>
        <div className="absolute bottom-3 right-3 chip bg-black/40 text-accent-2">{!ended ? `⏱ ${remaining}` : 'Ended'}</div>
      </div>
      <div className="p-4">
        <h3 className="font-display font-semibold leading-snug group-hover:text-accent transition pr-6">{listing.title}</h3>
        <p className="text-sm text-muted mt-0.5">{listing.city} {listing.verified && <span className="text-accent-2">✓</span>}</p>
        <div className="mt-3 flex items-end justify-between">
          <div>
            <p className="text-xs text-muted uppercase tracking-wide">{isReverse ? 'Now' : listing.currentBid > 0 ? 'Current bid' : 'Reserve'}</p>
